...
const PM = () => {
  ...
  const [sinWaveDataPM, setSinWaveDataPM] = useState(null);
    const [demodulation_pm, setdemodulation_pm] = useState(null);
...
const handleClickc = async () => {
  ...
  const handleClick = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/process_data_pm?frequency=${messageFrequency}&amplitude=${carrierFrequency}`
      );

      setSinWaveDataPM(response.data.sinWaveDataPM);
    } catch (error) {
      console.error('Error making request:', error);
    }
  };
  const handleClick1 = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/demodulation_pm?frequency=${messageFrequency}&amplitude=${carrierFrequency}`
      );
      setdemodulation_pm(response.data.demodulation_pm);
    } catch (error) {
      console.error('Error making request:', error);
    }
  };
...
<div>
  ...
  <label>
          ...
          {sinWaveData0 && (
		    ...
		    <label>
          <button onClick={handleClick1}>Demodulation</button>
          {demodulation_pm && (
            <div>
              <h3></h3>
              <Plot
        ...
        <br />
	            ...
	            <button onClick={handleClickc}>Generate</button>
